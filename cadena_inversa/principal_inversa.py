#PROGRAMA PRINCIPAL PARA DEVOLVER UNA CADENA INVERSA A UNA DADA COMO ENTRADA
import random
...
def generaciones(lista_entrada: list[str], tamaño_poblacion: int = 100, tamaño_programa: int = 20, 
                 max_generaciones: int = 1000, prob_cruce: float = 0.4, 
                 fraccion_elite: float = 0.02, tamaño_torneo: int = 2) -> tuple[str | None, float]:
    
    casos_prueba = [(s, s[::-1]) for s in lista_entrada]
...
#generaciones(['1234'])
#generaciones(['123', 'ab'])
#generaciones(['123', 'ab', 'M'])
#generaciones(['123', 'ab', 'M', ''])
#generaciones(['12345678', 'ab', 'M', ''])